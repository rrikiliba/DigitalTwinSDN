...
CMD4="clear; ryu-manager src/custom_switch_13.py ryu.app.rest_topology ryu.app.ofctl_rest --observe-links"
CMD5="clear; ryu-manager ryu.app.gui_topology ryu.app.ws_topology ryu.app.rest_topology ryu.app.ofctl_rest src/custom_switch_13.py --wsapi-port 6060 --ofp-tcp-listen-port 6666 --observe-links"

# Terminate previous session if found
tmux has-session -t $SESSION_NAME 2>/dev/null
if [ $? = 0 ]; then
  echo "Found existing session, terminating it and starting anew..."
...
tmux split-window -t $SESSION_NAME -h -p 66
tmux split-window -t $SESSION_NAME -h -p 50

# Panel 5: twin network topology and traffic checker
...
# Panel 2: ryu controller for the twin network
